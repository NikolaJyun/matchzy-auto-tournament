# Screenshots

Take a tour of MatchZy Auto Tournament's interface. Click any image to view in full size.

---

## Dashboard

Get started quickly with the onboarding checklist and quick access cards.

![Dashboard](assets/preview/01-dashboard.png)

---

## Tournament Setup

### Create Tournament

Configure your tournament with name, teams, maps, and format in a simple step-by-step form.

![Tournament Creation](assets/preview/02-tournament-creation.png)

### Complete Setup

Fill in all details including 8 teams and 7 maps for the competitive pool.

![Tournament Setup](assets/preview/06-tournament-setup.png)

### Review & Confirm

Review all settings before starting your tournament.

![Tournament Review](assets/preview/07-tournament-review.png)

---

## Team Management

Manage all your teams in one place. Create, edit, and view team rosters with player details.

![Teams Management](assets/preview/03-teams-management.png)

---

## Server Management

Configure and monitor your CS2 servers with real-time status indicators.

![Servers Management](assets/preview/04-servers-management.png)

---

## Tournament Bracket

Double-elimination bracket showcasing synchronized winner and loser brackets with seeded matchups.

![Bracket View](assets/preview/08-bracket-view.png)

---

## Match Management

### Matches Overview

Overview of all matches with live status updates and match cards.

![Matches Overview](assets/preview/09-matches-overview.png)

### Match Details Modal

Detailed match view with live player roster, admin controls, and match information.

![Match Details Modal](assets/preview/12-match-details-modal.png)

---

## Public Team Pages

### Map Veto Interface

Interactive FaceIT-style map veto for teams with all competitive maps.

![Team Map Veto](assets/preview/10-team-map-veto.png)

### Match Information

Teams can view match details, server connection info, and player rosters.

![Team Match Page](assets/preview/11-team-match-page.png)

---

## Admin Tools

Powerful RCON commands and match controls for tournament administrators.

![Admin Tools](assets/preview/05-admin-tools.png)

---

## Maps & Map Pools Management

### Maps Page

Manage all your maps with visual cards, add custom maps, and upload map images.

![Maps Page](assets/preview/13-maps-page.png)

### Map Pools Tab

Create reusable map pools for your tournaments. View all pools with map counts and previews.

![Map Pools Tab](assets/preview/14-map-pools-tab.png)

### Creating Map Pools

Select maps from your collection to create custom map pools for different tournament types.

![Map Pool Creation](assets/preview/15-map-pool-creation.png)

### Tournament Map Pool Selection

Choose from Active Duty, custom pools, or create a custom selection during tournament setup.

![Tournament Map Pool Selection](assets/preview/16-tournament-map-pool-selection.png)

---

## Key Features Shown

✅ **Onboarding System** - Step-by-step getting started guide  
✅ **Tournament Creation** - Multi-step form with validation  
✅ **Team Management** - Full roster management with team tags  
✅ **Server Management** - Real-time server status monitoring  
✅ **Visual Brackets** - Clean, modern bracket visualization  
✅ **Match Management** - Live match tracking and status updates  
✅ **Match Details** - Player roster with live connection status (0/10 players)  
✅ **Map Veto** - Interactive pick/ban system with map previews  
✅ **Maps & Map Pools** - Custom map management with reusable pools  
✅ **Public Team Pages** - No-auth pages for team access  
✅ **Admin Controls** - Comprehensive RCON command interface with match control buttons  

---

<div align="center" markdown>

**Ready to get started?**

[Quick Start Guide](getting-started/quick-start.md){ .md-button .md-button--primary }
[View Features](features/overview.md){ .md-button }

</div>

